...
class _MasterMessagesScreenState extends State<MasterMessagesScreen> {
  ...
  Future<void> _fetchConversations() async {
    try {
      ...
      // Fetch accepted requests where assigned_master_id = current user
      final requestsResponse = await Supabase.instance.client
          .from('requests')
          .select(
            'id, client_id, description, status, created_at, categories(name), users!requests_client_id_fkey(name)',
          )
      ...
      for (final request in requests) {
        ...
        final clientName =
            (request['users'] as Map<String, dynamic>?)?['name'] as String? ??
            'Client';
        ...
        try {
          // Fetch last message sorted by created_at DESC
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 1.h),
          Text(
            'Accepted leads will appear here',
...
class _MasterConversationTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const clientNameColor = Color(0xFF111111);
    const badgeBg = Color(0xFFF2F4F7);
    const badgeTextColor = Color(0xFF222222);
                        ...
                        Expanded(
                          child: Text(
                            ...
                            style: TextStyle(
                              fontSize: 20,
                              fontWeight: FontWeight.bold,
                              color: isDark ? Colors.white : clientNameColor,
                            ),
